...
//                // 解析符文
//                var (primary, secondary) = await LoadRunesAsync(stats);

//                int champId = selfParticipant["championId"].Value<int>();   //英雄ID
//                //根据英雄ID获取图片路径，名称，描述
//                var (champIcon, champName, champDesc) = await Globals.lcuClient.GetChampionInfoAsync(champId);

//                //召唤师1
//                int spell1Id = selfParticipant["spell1Id"].Value<int>();
//                var (spellImg1, spellName1, spellDesc1) = await Globals.lcuClient.GetSpellInfoAsync(spell1Id);

//                //召唤师2
//                int spell2Id = selfParticipant["spell2Id"].Value<int>();
//                var (spellImg2, spellName2, spellDesc2) = await Globals.lcuClient.GetSpellInfoAsync(spell2Id);
...
//                    ResultColor = isWin ? System.Drawing.Color.Green : System.Drawing.Color.Red,
...
//                    HeroIcon = champIcon,   //头像地址
//                    ChampionName = champName,   //名称
//                    ChampionDescription = champDesc,    //描述
//                    Kills = kills,
//                    Deaths = deaths,
//                    Assists = assists,
//                    SummonerSpells = new[]
...
//                        spellImg2   //召唤师2图片
//                    },
//                    SpellNames = new[] { spellName1, spellName2 },  //召唤师技能名称
//                    SpellDescriptions = new[] { spellDesc1, spellDesc2 },   //召唤师技能描述
//                    Items = await LoadItemsAsync(stats),    //装备图片及名称，描述
//                    DamageText = damageText,
//                    GoldText = goldText,
//                    KPPercentage = kpTextWithRank,

//                    PrimaryRunes = primary, //主系符文
//                    SecondaryRunes = secondary, //副系符文
...
//                FillMatchToolTips(match);   //构建 MatchInfo 后补充名称，描述展示字段

//                // 填充队伍信息
//                PopulateTeams(match, allParticipants, identities, participantId);
...
//        {
//            var primaryRunes = new List<RuneInfo>();
//            var secondaryRunes = new List<RuneInfo>();

//            //主系符文(perk0 - perk3)
//            for (int i = 0; i < 4; i++)
//            {
//                var runeId = stats[$"perk{i}"]?.Value<int>() ?? 0;
//                var item = Globals.lcuClient.GetRuneById(runeId);

//                if (item != null)
//                {
//                    var icon = await Globals.lcuClient.GetRuneInfoAsync(runeId); // 直接传原始 ID
//                    primaryRunes.Add(new RuneInfo
//                    {
//                        RuneId = runeId,
//                        Icon = icon.image,
//                        Name = icon.name,
...
//            for (int i = 0; i < 2; i++)
//            {
//                var runeId = stats[$"perk{i + 4}"]?.Value<int>() ?? 0;
//                var item = Globals.lcuClient.GetRuneById(runeId);

//                if (item != null)
//                {
//                    var icon = await Globals.lcuClient.GetRuneInfoAsync(runeId); // 直接传原始 ID
//                    secondaryRunes.Add(new RuneInfo
//                    {
//                        RuneId = runeId,
//                        Icon = icon.image,
//                        Name = icon.name,
...
//                else
//                {
//                    secondaryRunes.Add(null);
//                }
//            }

//            return (primaryRunes.ToArray(), secondaryRunes.ToArray());
//        }
...
//                if (itemId > 0)
//                {
//                    var (image, name, desc) = await Globals.lcuClient.GetItemInfoAsync(itemId);
//                    list.Add(new Item
//                    {
//                        Id = itemId,
//                        Icon = image,
...
//        private async Task PopulateTeams(MatchInfo match, List<JObject> participants, List<JObject> identities, int selfParticipantId)
//        {
...
//                int champId = p["championId"].Value<int>();   //英雄ID
//                //根据英雄ID获取图片路径，名称，描述
//                var (champIcon, champName, champDesc) = await Globals.lcuClient.GetChampionInfoAsync(champId);
...
//                    //Avatar = ImageLoader.GetChampionIconAsync(p["championId"].Value<int>()).Result, // 同步加载简化示例
//                    Avatar = champIcon, // 同步加载简化示例
//                    TeamId = p["teamId"].Value<int>(),
//                    IsSelf = pid == selfParticipantId
//                };

//                if (player.TeamId == 100)
//                    match.BlueTeam.Add(player);
//                else if (player.TeamId == 200)
//                    match.RedTeam.Add(player);
//            }
//        }
//        #endregion
        ...
        public async Task<Panel> ParseGameToPanelAsync(JObject game, string gameName, string tagLine)
        {
            try
            {
                ...
                var spell1Task = LoadWithSemaphore(() =>
                    Globals.lcuClient.GetSpellInfoAsync(selfParticipant["spell1Id"].Value<int>()));

                var spell2Task = LoadWithSemaphore(() =>
                    Globals.lcuClient.GetSpellInfoAsync(selfParticipant["spell2Id"].Value<int>()));
        ...
        private async Task<(RuneInfo[] Primary, RuneInfo[] Secondary)> LoadRunesAsync(JToken stats)
        {
            var primaryTasks = Enumerable.Range(0, 4).Select(async i =>
            {
                ...
                try
                {
                    ...
                    var icon = await Globals.lcuClient.GetRuneInfoAsync(runeId);
                    return new RuneInfo
                    {
                        RuneId = runeId,
                        Icon = icon.image,

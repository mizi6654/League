...
namespace League.uitls
{
    public class ClosableTabControl : TabControl
    {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            for (int i = 0; i < TabCount; i++)
            {
                if (_closeButtonBounds.TryGetValue(i, out Rectangle rect) && rect.Contains(e.Location))
                {
                    //if (TabPages[i] is MatchTabPage tabPage)
                    //{
                    //    tabPage.LoadCts.Cancel(); // 取消正在进行的加载
                    //    tabPage.Dispose(); // 确保资源释放
                    //}
